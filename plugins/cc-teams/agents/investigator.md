...
Read(file_path=".claude/cc-teams/patterns.md")  # Check Common Gotchas!
...
- External service/API bugs → `Skill(skill="cc-teams:github-research")` (falls back to WebFetch if unavailable)
- 3+ local debugging attempts failed → `Skill(skill="cc-teams:github-research")` for external research
...
4. **Upon completion:** Output Router Contract with STATUS and HYPOTHESIS_VERDICT
5. **Non-response consequence:** At T+8, lead spawns replacement investigator and reassigns task
