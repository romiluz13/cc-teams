...
```
CC-TEAMS DEBUG: {error_summary}
├── CC-TEAMS investigator-1: Test hypothesis - {h1}
├── CC-TEAMS investigator-2: Test hypothesis - {h2}
├── CC-TEAMS investigator-3: Test hypothesis - {h3}
├── CC-TEAMS Bug Court: Debate round (blocked by all investigators)
├── CC-TEAMS builder: Implement winning fix (blocked by debate)
├── CC-TEAMS security-reviewer: Review fix security (blocked by fix)
├── CC-TEAMS performance-reviewer: Review fix performance (blocked by fix)
├── CC-TEAMS quality-reviewer: Review fix quality (blocked by fix)
├── CC-TEAMS DEBUG Review Arena: Challenge round (blocked by all 3 reviewers)
├── CC-TEAMS verifier: Verify fix E2E (blocked by challenge round)
└── CC-TEAMS Memory Update: Persist debug learnings (blocked by verifier)
```
