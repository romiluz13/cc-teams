...
```
TaskCreate({
  subject: "CC-TEAMS Execute Plan: {feature}",
...
Update `.claude/cc-teams/progress.md` with task *subjects* (and optionally task IDs for the current session).
...
- CC-TEAMS Execute Plan: {feature} (blocked by: -)
- CC-TEAMS Phase 1: {title} (blocked by: -)
- CC-TEAMS Phase 2: {title} (blocked by: CC-TEAMS Phase 1: {title})
")
