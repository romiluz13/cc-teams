...
**Timeout escalation (per cc-teams-lead escalation ladder):**
...
```
CC-TEAMS BUILD: {feature}
├── CC-TEAMS builder: Implement {feature}
├── CC-TEAMS live-reviewer: Real-time review (starts with builder)
├── CC-TEAMS hunter: Silent failure audit (blocked by builder)
...
├── CC-TEAMS BUILD Review Arena: Challenge round (blocked by all 3 reviewers)
├── CC-TEAMS verifier: E2E verification (blocked by challenge round)
└── CC-TEAMS Memory Update: Persist build learnings (blocked by verifier)
```
...
1. Lead creates `CC-TEAMS REM-FIX: {issue}` task
...
All notes merged and persisted to `.claude/cc-teams/` files.
