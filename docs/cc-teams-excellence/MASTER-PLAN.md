# CC-Teams Excellence Program (Production Governance)

## 0) Purpose
Operate and improve CC-Teams in production with strict, evidence-backed upgrades while preserving CC10x core DNA.
...
- CC-Teams is live in production on Agent Teams foundations.
...
3. **Router remains the only entry point** (`cc-teams-lead`).
...
## Present in repo
1. `docs/cc-teams-excellence/MASTER-PLAN.md`
2. `docs/cc-teams-excellence/KPI-SCORECARD.md`
3. `docs/cc-teams-excellence/BENCHMARK-CASES.md`
4. `docs/cc-teams-excellence/DECISION-LOG.md`
5. `archive/2026-02-11-post-launch-docs/docs/cc-teams-excellence/BORROW-STRATEGY-NEURAL-HARMONY.md` (historical strategy snapshot)
6. `docs/cc-teams-excellence/PRODUCTION-READINESS-SYSTEM.md`
7. `docs/cc-teams-excellence/POST-LAUNCH-HARDENING-PLAN-2026-02-11.md`
8. Runtime hardening across:
   - `plugins/cc-teams/skills/cc-teams-lead/SKILL.md`
   - `plugins/cc-teams/skills/router-contract/SKILL.md`
   - `plugins/cc-teams/skills/review-arena/SKILL.md`
   - `plugins/cc-teams/skills/bug-court/SKILL.md`
   - `plugins/cc-teams/skills/pair-build/SKILL.md`
   - `plugins/cc-teams/agents/*.md`

## Newly implemented for Phase D foundation
1. `scripts/eval/run-benchmark.sh`
2. `scripts/eval/score-benchmark.sh`
3. `scripts/lint-cc-teams-protocol-integrity.sh`
...
Add profile selection in `cc-teams-lead`:
...
- `plugins/cc-teams/skills/bug-court/SKILL.md`
- `plugins/cc-teams/skills/pair-build/SKILL.md`
- `plugins/cc-teams/agents/*.md` (contract output sections)
...
Deliver:
- benchmark scripts and baseline runs CC10x vs CC-Teams
...
CC-Teams can be called best-in-class only if:
...
1. Run `npm run check:cc-teams` before and after any behavior-affecting change.
...
"Use `docs/cc-teams-excellence/MASTER-PLAN.md` as the single source of truth for production governance. Execute targeted audits, run `npm run check:cc-teams`, and update decision evidence before shipping any behavior change."
